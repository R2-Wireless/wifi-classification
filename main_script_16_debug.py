...
DEFAULT_CENTER_FREQ_HZ = 5.180e9
CHANNEL_BW_TO_RATE = {5: 5e6, 10: 10e6, 20: 20e6}
DEFAULT_INPUT_RATE_BY_EXT = {
    '.iq': 40e6,
    '.mat': 30.72e6,
    '.bin': 30.72e6,
    '.cfile': 20e6,
...
def _default_input_rate_for_path(path: str) -> Optional[float]:
    return DEFAULT_INPUT_RATE_BY_EXT.get(_normalize_ext(path))


def _format_rate_mhz(rate_hz: float) -> str:
    return f"{float(rate_hz) / 1e6:.2f} MHz"


def _resolve_target_rate_hz(options) -> float:
    target_rate = getattr(options, 'target_rate', None)
    channel_bw = getattr(options, 'channel_bw', None)
    if target_rate is None and channel_bw is None:
        return 20e6
    if target_rate is None:
        return float(CHANNEL_BW_TO_RATE[int(channel_bw)])
    target_rate = float(target_rate)
    if channel_bw is not None:
        expected = float(CHANNEL_BW_TO_RATE[int(channel_bw)])
...
def _resolve_input_rate_hz(input_path: str, explicit_rate: Optional[float] = None) -> Optional[float]:
    if explicit_rate is not None:
        return float(explicit_rate)
    return _default_input_rate_for_path(input_path)

def _maybe_store_capture_rate(capture: dict, samp_rate_hz: float) -> dict:
    if capture is None:
        return capture
    capture['target_samp_rate_hz'] = float(samp_rate_hz)
    capture.setdefault('capture_samp_rate_hz', float(samp_rate_hz))
    return capture
...
class wifi_rx_sync_long_only(gr.top_block):
    def __init__(
        ...
        use_throttle=True,
        debug_replay_tags=False,
    ):
        gr.top_block.__init__(self, "WiFi RX Sync Long Only")

        self.freq = freq
        self.samp_rate = samp_rate
        self.debug_replay_tags = bool(debug_replay_tags)
    ...
    @staticmethod
    def _capture_debug_rows(capture):
        rows = []
        offsets = np.asarray(capture.get("tag_offsets", []), dtype=np.uint64)
        keys = np.asarray(capture.get("tag_keys", []), dtype=object)
        val_types = np.asarray(capture.get("tag_value_types", []), dtype=object)
        vals_f64 = np.asarray(capture.get("tag_values_f64", []), dtype=np.float64)
        vals_u64 = np.asarray(capture.get("tag_values_u64", []), dtype=np.uint64)

        by_offset = {}
        for idx, off in enumerate(offsets.tolist()):
            d = by_offset.setdefault(int(off), {})
            key = str(keys[idx])
            typ = str(val_types[idx])
            if typ == "u64":
                d[key] = int(vals_u64[idx])
            else:
                d[key] = float(vals_f64[idx])

        for off in sorted(by_offset):
            d = by_offset[off]
            if "frame_id" in d or "wifi_start" in d or "cfo_long_rad_per_samp" in d:
                rows.append({
                    "offset": off,
                    "frame_id": d.get("frame_id"),
                    "wifi_start": d.get("wifi_start"),
                    "cfo_long_rad_per_samp": d.get("cfo_long_rad_per_samp"),
                    "cfo_short_rad_per_samp": d.get("cfo_short_rad_per_samp"),
                })
        return rows

    def _debug_print_capture_summary(self, capture, inject_tags, sample_count):
        rate_hz = float(capture.get("target_samp_rate_hz", self.samp_rate))
        print(f"[replay-debug] sample_count={sample_count:,}  tag_count={len(inject_tags)}  samp_rate={rate_hz/1e6:.3f} MHz")
        rows = self._capture_debug_rows(capture)
        if not rows:
            print("[replay-debug] no frame-level replay tags found")
            return
        print("[replay-debug] frame tags:")
        print("[replay-debug] +--------+------------+----------------------+----------------------+----------------------+")
        print("[replay-debug] | frame_id| offset     | wifi_start           | cfo_long             | cfo_short            |")
        print("[replay-debug] +--------+------------+----------------------+----------------------+----------------------+")
        for row in rows:
            frame_id = "" if row["frame_id"] is None else str(int(row["frame_id"]))
            wifi_start = "" if row["wifi_start"] is None else f"{float(row['wifi_start']): .9e}"
            cfo_long = "" if row["cfo_long_rad_per_samp"] is None else f"{float(row['cfo_long_rad_per_samp']): .9e}"
            cfo_short = "" if row["cfo_short_rad_per_samp"] is None else f"{float(row['cfo_short_rad_per_samp']): .9e}"
            print(f"[replay-debug] | {frame_id:6s} | {row['offset']:10d} | {wifi_start:20s} | {cfo_long:20s} | {cfo_short:20s} |")
        print("[replay-debug] +--------+------------+----------------------+----------------------+----------------------+")
    ...
    def load_capture(self, capture):
        ...
        if self.debug_replay_tags:
            self._debug_print_capture_summary(capture, inject_tags, len(samples))
...
class wifi_rx_file(gr.top_block):
    def __init__(self, filename, output_pcap, freq_offset=0.0, verbose=True,
                 use_sync_combined=False, sts_periods=1, samp_rate=20e6, freq=DEFAULT_CENTER_FREQ_HZ):
...
def argument_parser():
    ...
    parser.add_argument(
        "input_file",
        help=(
            "Input IQ file (.cfile complex64, .iq complex64 at true 40 MHz, "
            "or .mat/.bin at true 30.72 MHz). The file is resampled to the native "
            "target Wi-Fi rate selected by --channel-bw / --target-rate."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override the true input sample rate in Hz. Defaults: .iq -> 40e6, "
            ".mat/.bin -> 30.72e6, .cfile -> 20e6. The data is resampled to the "
            "native target Wi-Fi rate before processing."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="In --sync-long-only mode, print frame_id and replay tag values (wifi_start / cfo_long / cfo_short) before starting the flowgraph.",
    )
    return parser
...
def _build_sync_long_cfg(options) -> SyncLongConfig:
    target_rate_hz = _resolve_target_rate_hz(options)
    return SyncLongConfig(
        samp_rate=target_rate_hz,
        expected_gap=64,
...
def _probe_input_sample_count(input_path: str, samp_rate_in: Optional[float] = None, target_rate_hz: float = 20e6) -> Optional[int]:
    if input_path.lower().endswith((".cfile", ".iq")):
        try:
            ...
            if input_path.lower().endswith(".iq"):
                ...
                samp_rate_in_eff = _resolve_input_rate_hz(input_path, samp_rate_in) or 40e6
                samp_rate_out = target_rate_hz
    ...
    if input_path.lower().endswith(".mat"):
        try:
            ...
            samp_rate_in = _resolve_input_rate_hz(input_path, samp_rate_in) or 30.72e6
            samp_rate_out = target_rate_hz
            up = int(round(samp_rate_out))
...
def _load_sync_long_iq_data(options, prep_rows):
    ...
    target_rate_hz = float(cfg.samp_rate)
    t0 = time.perf_counter_ns()
    if input_path.lower().endswith(".mat"):
        samp_rate_in = _resolve_input_rate_hz(input_path, options.samp_rate_in) or 30.72e6
        print(
            f"[sync_long_only] Loading .mat file (input rate={_format_rate_mhz(samp_rate_in)}, "
            f"resampling to {_format_rate_mhz(target_rate_hz)})..."
        )
        iq_data = load_mat_file(input_path, samp_rate_in=samp_rate_in, samp_rate_out=target_rate_hz)
    elif input_path.lower().endswith(".iq"):
        samp_rate_in = _resolve_input_rate_hz(input_path, options.samp_rate_in) or 40e6
        print(
            f"[sync_long_only] Loading .iq file (input rate={_format_rate_mhz(samp_rate_in)}, "
            f"resampling to {_format_rate_mhz(target_rate_hz)})..."
        )
        iq_data = load_iq_file(input_path, samp_rate_in=samp_rate_in, samp_rate_out=target_rate_hz)
    else:
...
def _mat_to_tmp_cfile(mat_path: str, samp_rate_in: float = 30.72e6, samp_rate_out: float = 20e6) -> str:
    """Convert a .mat IQ file to a temporary complex64 .cfile at the target rate."""
    iq_data = load_mat_file(mat_path, samp_rate_in=samp_rate_in, samp_rate_out=samp_rate_out)
...
def _iq_to_tmp_cfile(iq_path: str, samp_rate_in: float = 40e6, samp_rate_out: float = 20e6) -> str:
    """Convert a .iq complex64 file to a temporary complex64 .cfile at the target rate."""
    iq_data = load_iq_file(iq_path, samp_rate_in=samp_rate_in, samp_rate_out=samp_rate_out)
...
def _bin_to_tmp_cfile(bin_path: str, samp_rate_in: float = 30.72e6, samp_rate_out: float = 20e6) -> str:
    """Convert a raw complex64 .bin IQ file to a temporary complex64 .cfile at the target rate."""
    iq_data = load_iq_file(bin_path, samp_rate_in=samp_rate_in, samp_rate_out=samp_rate_out)
    tmp = tempfile.NamedTemporaryFile(suffix=".cfile", delete=False, prefix="bin_iq_")
    iq_data.tofile(tmp)
    tmp.close()
    return tmp.name
...
def main(top_block_cls=wifi_rx_file, options=None):
    ...
    prewarm_sample_count = (
        _probe_input_sample_count(options.input_file, getattr(options, "samp_rate_in", None), target_rate_hz=target_rate_hz)
    ...
    print(f"Channel BW / target rate: {options.channel_bw} MHz / {_format_rate_mhz(target_rate_hz)}")
    ...
    input_path = options.input_file
    if not options.sync_long_only:
        input_ext = _normalize_ext(input_path)
        if input_ext == '.mat':
            samp_rate_in = _resolve_input_rate_hz(input_path, options.samp_rate_in) or 30.72e6
            print(f"[mat] Converting .mat -> temp .cfile (resample {_format_rate_mhz(samp_rate_in)} -> {_format_rate_mhz(target_rate_hz)})...")
            temp_input_path = _mat_to_tmp_cfile(input_path, samp_rate_in=samp_rate_in, samp_rate_out=target_rate_hz)
            print(f"[mat] Temp file: {temp_input_path}")
            input_path = temp_input_path
        elif input_ext == '.iq':
            samp_rate_in = _resolve_input_rate_hz(input_path, options.samp_rate_in) or 40e6
            print(f"[iq] Converting .iq -> temp .cfile (resample {_format_rate_mhz(samp_rate_in)} -> {_format_rate_mhz(target_rate_hz)})...")
            temp_input_path = _iq_to_tmp_cfile(input_path, samp_rate_in=samp_rate_in, samp_rate_out=target_rate_hz)
            print(f"[iq] Temp file: {temp_input_path}")
            input_path = temp_input_path
        elif input_ext == '.bin':
            samp_rate_in = _resolve_input_rate_hz(input_path, options.samp_rate_in) or 30.72e6
            print(f"[bin] Converting .bin -> temp .cfile (resample {_format_rate_mhz(samp_rate_in)} -> {_format_rate_mhz(target_rate_hz)})...")
            temp_input_path = _bin_to_tmp_cfile(input_path, samp_rate_in=samp_rate_in, samp_rate_out=target_rate_hz)
            print(f"[bin] Temp file: {temp_input_path}")
            input_path = temp_input_path
    ...
    if options.sync_long_only:
        ...
        capture = _maybe_store_capture_rate(prep_result.capture, target_rate_hz)
        ...
        tb = wifi_rx_sync_long_only(
            ...
            debug_replay_tags=bool(getattr(options, "debug_replay_tags", False)),
        )
    else:
        tb = top_block_cls(
            filename=input_path,
